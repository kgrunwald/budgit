...
Parse.Cloud.afterSave(
  ...
  async (req): Promise<void> => {
    ...
    if (origCatId) {
      ...
      console.log(
        '[CLOUD] No previous category for txn. Updating account balance.'
      );
...
const setCategoryActivity = async (txn: Transaction) => {
  // @ts-ignore
  const ctg = (await new Parse.Query(Category).get(txn.category.id, {
    useMasterKey: true,
  })) as Category;
  if (!ctg) {
  ...
  // @ts-ignore
  const acct = (await new Parse.Query(Account).get(txn.account.id, {
    useMasterKey: true,
  })) as Account;
  if (ctg.isPayment) {
    console.log(
      `[CLOUD] Transaction ${txn.transactionId} is marked as a payment. Looking up corresponding account.`
    );
...
const setCreditActivity = async (txn: Transaction, acct: Account) => {
  ...
  // @ts-ignore
  const paymentCtg = await new Parse.Query(Category)
...
const updateAccountBalance = async (txn: Transaction) => {
  ...
  if (acct.currentBalance !== balance) {
    console.log(
      `[CLOUD] Setting account balance for ${acct.accountId}: ${balance}`,
      balance
    );
